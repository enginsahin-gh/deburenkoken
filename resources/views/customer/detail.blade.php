...
@section('content')
<style>
    .order-button, .order-button.out-of-order {
    ...
    <div class="breadcrumbs">
        <div class="container">
            ...
            <a href="{{route('search.coordinates', [$searchString])}}">Terug</a>
        </div>
    </div>
    ...
    <div class="container single-dish">
        <div class="row">
                ...
                @if (!is_null($user))
                    @if ($user->cook?->getUuid() === $advert->cook->getUuid())
                        <div class="row">
                            ...
                            @else
                                <div class="col-12">
                                    <span class="alert alert-warning" style="display: block;">Een advertentie kan niet gewijzigd/geannuleerd worden na het uiterlijke bestelmoment.</span>
                                </div>
                            @endif
                        </div>
                    @endif
            ...
            <div class="col-3 line">
                <a href="{{route('search.cooks.detail', $advert->cook->getUuid())}}" class="d-block align-center justify-center">
                    <div class="row">
                    ...
                    <div class="col-12">
                        ...
                        @else
                            <button class="order-button out-of-order">
                                @if ($advert->getParsedOrderTo()->isFuture() && $advert->getLeftOverAmount() === 0) Uitverkocht @else uiterlijke bestelmoment verlopen @endif
                            </button>
                        @endif
                        <div class="text-left afhalen">
                            <b>Datum afhalen:</b><br/>
                            {{$advert->getParsedPickupFrom()->translatedFormat('d-m-Y / H:i -')}} {{$advert->getParsedPickupTo()->translatedFormat('H:i')}}
                        </div>
                        <div class="d-flex justify-left">
                            <div class="distance">
                                <i class="justify-left fa fa-map-marker"></i> {{strtok($distance, '.')}} km
                            </div>
                        </div>
                    </div>
                </div>
