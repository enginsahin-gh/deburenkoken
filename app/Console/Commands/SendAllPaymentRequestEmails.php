...
class SendAllPaymentRequestEmails extends Command
{
    protected $signature = 'emails:send-all-payment-requests';
    ...
    public function handle()
    {
        ...
        $usersPayments = Payment::whereNull('payment_date')->get();

        if ($usersPayments->isEmpty()) {
            $this->info('Er zijn geen Uitbetalingsverzoeken ingediend.');
        ...
        $html = '<!DOCTYPE html>
                    ...
                    <body>
                        <h2>Uitbetalings Verzoeken - Nog goed te keuren - Overzicht - DATE</h2>
                                USERS
                        </table>
                    </body>
                    </html>';
        ...
        foreach ($adminUsers as $adminUser) {
            Mail::to(
                $adminUser->email,
                $adminUser->username
            )->send(new PaymentRequestAllMail(
                $adminUser,
